...
- newCrisis: 仅当上方没有未决突发事件,且本回合确实造成或暴露了新的重大麻烦时才抛出;它不是每回合必填。若局势刚刚解决危机、没有自然产生的新威胁,返回 null。生成时必须是会自己倒计时、有明确量化代价的新麻烦,deadline 由系统设定,你只填 title/summary/source/severity/penalty。penalty 是危机逾期后每回合扣除的绝对值,四项都只能填 0 到 25 的非负数;需要扣 9 就填 9,绝不能填 -9。只有 deltas 表示本回合即时变化,才允许使用正负数。
